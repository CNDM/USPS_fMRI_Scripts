function theData = Build_fmrAdapt2Regressor(s,overwrite)
...
%---------------------------------------------------------------
    ...
    adapt.PairGroup = [cellstr(adapt1.PairGroup);cellstr(adapt2.PairGroup)];
        ...
        Header = {'CategoryOnset','BrandOffset','MemoryOnset','MemoryRT','Type','Format'};
        CategoryOnset = timeTotal(:,1);
        BrandOffset = timeTotal(:,4)-timeTotal(:,1);
        ...
        Recognition = [CategoryOnset,BrandOffset,MemoryOnset,MemoryRT,Type',Format'];
        ...
        if overwrite ==1
            csvwrite(fullfile(saveDir,subjectID,sprintf('fmrAdapt2Regressor%s',subjectID)),Recognition)
        else
            if exist(fullfile(saveDir,subjectID,sprintf('fmrAdapt2Regressor%s',subjectID)))==2
