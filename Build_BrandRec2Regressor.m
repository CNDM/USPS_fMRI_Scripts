function theData = Build_BrandRec2Regressor(s)
...
%---------------------------------------------------------------
    ...
    BrandRec = tdfread(sprintf('BrandRec_withPost.csv'),',')
        
        Header = {'BrandOnset','BrandDuration','RememberOnset','RememberDuration','VividOnset','VividDuration','VividResponse','Type','Format','PostScore'};
        ...
        BrandRec = [BrandOnset,BrandDuration,RememberOnset,RememberDuration,VividOnset,VividDuration,VividResponse,Type',Format',PostScore]
        ...
        if exist(fullfile(saveDir,subjectID,sprintf('BrandRec2Regressor.%s',subjectID)))==2
            overwrite = input('WARNING: This file already exists \n Would you like to Overwrite it? (y/n)\n','s');
            if overwrite=='n'
                cd(cwd)
                return
            else
            csvwrite(fullfile(saveDir,subjectID,sprintf('BrandRec2Regressor%s',subjectID)),BrandRec)
            end
        else
            csvwrite(fullfile(saveDir,subjectID,sprintf('BrandRec2Regressor%s',subjectID)),BrandRec)
        end
