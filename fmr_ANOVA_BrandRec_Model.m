function PreProcessed = fmr_ANOVA_BrandRec_Model(fmr,pmod)
...
if pmod
    fmr.Model.(modelName).anova.directory = [fmr.secondLevelDir '/anova/' modelName 'pmod_' num2str(length(fmr.sub))];
else
...
for c = [16:21]
    ...
    for sub=1:length(fmr.sub)
        ...
        if c<10
            scans1(sub,c-15)={[modeldir '/con_000' num2str(c) '.nii,1']};
        else
            scans1(sub,c-15)={[modeldir '/con_00' num2str(c) '.nii,1']};
        end
    end %subj loop
end
for c = 1:6
    ...
    for sub=1:length(fmr.sub)
        modeldir = fullfile(fmr.Subject(sub).Scans.Functional.(model).directory,modelName);
        if c<10
            scans2(sub,c)={[modeldir '/con_000' num2str(c) '.nii,1']};
        else
            scans2(sub,c)={[modeldir '/con_00' num2str(c) '.nii,1']};
        end
    end %subj loop
end
